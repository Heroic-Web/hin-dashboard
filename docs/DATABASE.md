...
Complete reference for hin's PostgreSQL database schema using Prisma ORM.

## Overview

hin uses PostgreSQL with Prisma ORM for type-safe database access. The schema is designed for a multi-tenant affiliate marketing platform.
...
| `productName` | String | Product/service name | "Product Name" | "hin" |
| `programName` | String | Program display name | "Affiliate Program" | "hin Partners" |
| `websiteUrl` | String | Main website URL | "https://example.com" | "https://hin.com" |
...
async function main() {
  ...
  await prisma.programSettings.create({
    data: {
      productName: 'hin',
      programName: 'hin Partners',
      websiteUrl: 'https://hin.com',
      currency: 'USD',
...
**Database Documentation by hin Team**
