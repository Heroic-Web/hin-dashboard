...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    <body>
      ...
      <div class="footer">
        <p>This report was sent from hin by ${user.name} (${user.email})</p>
    ...
    const fromEmail = process.env.RESEND_FROM_EMAIL || 'hin <noreply@hin.com>';
    const results = await Promise.allSettled(
      recipients.map((email: string) =>
        resend.emails.send({
          from: fromEmail,
          to: email.trim(),
          subject: `[hin] ${reportData.type || 'Report'} — ${reportDate}`,
          html,
        })
      )
    );
