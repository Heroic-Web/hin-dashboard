...
class EmailService {
  private defaultFrom = process.env.RESEND_FROM_EMAIL || 'hin <noreply@hin.com>';
  ...
  private generateWelcomeEmailHTML(data: WelcomeEmailData): string {
    ...
    <body>
      ...
      <div class="content">
        ...
        <p>Best regards,<br>The hin Team</p>
      </div>
      <div class="footer">
        <p>This email was sent to ${data.email}</p>
        <p>© ${new Date().getFullYear()} hin. All rights reserved.</p>
      </div>
    </body>
    </html>
    `;
  ...
  private generateReferralNotificationHTML(data: ReferralNotificationData): string {
    ...
    <body>
      ...
      <div class="content">
        ...
        <p>Best regards,<br>The hin System</p>
      </div>
    </body>
    </html>
    `;
  ...
  async sendWelcomeEmail(data: WelcomeEmailData): Promise<{ success: boolean; message: string }> {
    try {
      const result = await resend.emails.send({
        ...
        to: data.email,
        subject: `Welcome to hin - ${data.role === 'affiliate' ? 'Affiliate' : 'Admin'} Account Created`,
  ...
  async sendPasswordResetEmail(email: string, resetToken: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      const result = await resend.emails.send({
        from: this.defaultFrom,
        to: email,
        subject: 'Password Reset Request - hin',
        html,
      });
  ...
  async sendVerificationEmail(email: string, verificationToken: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      const result = await resend.emails.send({
        from: this.defaultFrom,
        to: email,
        subject: 'Verify Your Email Address - hin',
        html,
      });
